...
void dijkstra (int arr[100][100], int vertex, int edge){ //fungsi dijkstranya
	...
	if (hasil[akhir-1]==MAX){
		...
		int x;
		if(akhir!=pilih){
			for(x=0; x<rute[akhir-1].size(); x++){
				if(x!=rute[akhir-1].size()-1){
					printf("%d -> ", rute[akhir-1].at(x)+1);
				}
				else{
					printf("%d\n\n", rute[akhir-1].at(x)+1);
				}
...
int main(){
	int vertex, edge, i, j, from, to, weight, a, start, hitung;
	int arr[100][100];
	hitung=0;
	printf("Welcome!\n");
			printf("1. Mulai\n2. Keluar\n");
	...
	while(1){
		...
		while(1){
			...
			while(1){
				...
				else if (start==3) {dijkstra(arr, vertex, edge); break;}
				else printf("Input error\n");
			}
			if(start==1) break;
		}
